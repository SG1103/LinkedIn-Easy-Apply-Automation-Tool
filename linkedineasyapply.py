...
class LinkedinEasyApply:
    ...
    def start_applying(self):
        ...
        for (position, location) in searches:
            ...
            if position is None:
                print("Error: position is None.")
            elif location is None:
                print("Error: location is None.")
            else:
    ...
    def next_job_page(self):
        linkedinJobLinks = urlgen.LinkedinUrlGenerate.generateUrlLinks()

        if linkedinJobLinks:
            random_url = random.choice(linkedinJobLinks)
            self.browser.get(random_url)
            time.sleep(10)
        else:
